...
class GraphConvLSTM(GraphConvRNN):
    ...
    2015 - Shi, Chen,Wang, Yeung - 'Convolutional LSTM Network:
...
class AttentionGraphConvLSTM(GraphConvRNN):
    ...
    def __init__(self,
                 ...
                 recurrent_dropout=0.,
                 **kwargs):
        cell = AttentionGraphConvLSTMCell(
            graph_data=graph_data,
            filters=filters,
            data_format=data_format,
            activation=activation,
            ...
            dropout=dropout,
            recurrent_dropout=recurrent_dropout)
        super(AttentionGraphConvLSTM, self).__init__(
            cell,
            return_sequences=return_sequences,
            go_backwards=go_backwards,
            stateful=stateful,
    ...
    def call(self, inputs, mask=None, training=None, initial_state=None):
        return super(AttentionGraphConvLSTM, self).call(
            inputs,
            mask=mask,
            training=training,
            initial_state=initial_state)
    ...
    def get_config(self):
        config = {'filters': self.filters,
                  ...
                  'dropout': self.dropout,
                  'recurrent_dropout': self.recurrent_dropout}
        base_config = super(AttentionGraphConvLSTM, self).get_config()
        del base_config['cell']
        return dict(list(base_config.items()) + list(config.items()))
...
class BaseGraphConvLSTMCell(Layer):
    ...
    def __init__(self,
                 ...
                 recurrent_dropout=0.,
                 **kwargs):
        super(BaseGraphConvLSTMCell, self).__init__(**kwargs)
        ...
        if data_format == 'channels_first':
            raise ValueError(
                'Invalid data format. Only `channels_last` is allowed.')
    ...
    def build(self, input_shape):
        # Build kernels:
        channel_axis = self.get_channels_axis(self.data_format, input_shape)
        ...
        # Build graph:
        self.build_graph(self.graph_data)

        super(BaseGraphConvLSTMCell, self).build(input_shape)

    def get_channels_axis(self, data_format, input_shape=None):
        if data_format == 'channels_first':
            channels_axis = -2
        elif data_format == 'channels_last':
            channels_axis = -1

        if (input_shape is not None) and (input_shape[channels_axis] is None):
            raise ValueError('The channels dimension of the inputs '
                             'should be defined. Found `None`.')
        return channels_axis

    def get_features_axis(self, data_format, input_shape=None):
        if data_format == 'channels_first':
            features_axis = -1
        elif data_format == 'channels_last':
            features_axis = -2

        if (input_shape is not None) and (input_shape[features_axis] is None):
            raise ValueError('The features dimension of the inputs '
                             'should be defined. Found `None`.')
        return features_axis
    ...
    def input_conv(self, x, w, b=None):
        conv_out = self.graph_convolution(x, w,
                                          data_format=self.data_format)
    ...
    def recurrent_conv(self, x, w):
        return self.graph_convolution(x, w,
                                      data_format=self.data_format)

    def graph_convolution(self, x, kernel, data_format=None):
        raise NotImplementedError()
...
class GraphConvLSTMCell(BaseGraphConvLSTMCell):
    """
    Implementation as per:
    2015 - Shi, Chen,Wang, Yeung - 'Convolutional LSTM Network:
    A Machine Learning Approach for Precipitation Nowcasting'
    ...
    def build_graph(self, graph_data):
        graph = utils.get_graph(graph_data)
...
class AttentionGraphConvLSTMCell(BaseGraphConvLSTMCell):
    ...
    def __init__(self,
                 ...
                 recurrent_dropout=0.,
                 **kwargs):
        super(AttentionGraphConvLSTMCell, self).__init__(
            graph_data=graph_data,
            filters=filters,
            data_format=data_format,
            activation=activation,
        ...
        # W, Wh, Wq, Us = None
        # bs, bu = None
        self.att_kernel_initializer = initializers.get('glorot_uniform')
        self.att_bias_initializer = initializers.get('zeros')

        self.att_kernel_regularizer = regularizers.get(None)
        self.att_bias_regularizer = regularizers.get(None)

        self.att_kernel_constraint = constraints.get(None)
        self.att_bias_constraint = constraints.get(None)

    def build(self, input_shape):
        # W, Wh, Wq, Us = None
        # bs, bu = None
        features_axis = self.get_features_axis(self.data_format, input_shape)
        num_features = input_shape[features_axis]
        attention_shape = (num_features, self.filters * 4)
        ...
        self.attention_kernel_w = self.attention_kernel[:, :self.filters]
        self.attention_kernel_wh = self.attention_kernel[:, self.filters:self.filters * 2]
        self.attention_kernel_wq = self.attention_kernel[:, self.filters * 2:self.filters * 3]
        self.attention_kernel_us = self.attention_kernel[:, self.filters * 3:self.filters * 4]

        self.attention_bias = self.add_weight(
            shape=(self.filters * 2, ),
            name='attention_bias',
            initializer=self.att_bias_initializer,
            regularizer=self.att_bias_regularizer,
            constraint=self.att_bias_constraint)

        self.attention_bias_bs = self.attention_bias[:self.filters]
        self.attention_bias_bu = self.attention_bias[self.filters:self.filters * 2]

        super(AttentionGraphConvLSTMCell, self).build(input_shape)
    ...
    def attention(self, HLt):
        """
        The intermediate hidden state `HLt` of AGC-LSTM contains rich spatial
        structural information and temporal dynamics that are beneficial in
        guiding the selection of key joints. So we first aggregate the
        information of all nodes as a query feature:
        ```
        qt = ReLU(sum(W * HLti))
        ```
        where `W` is the learnable parameter matrix.

        Then the attention scores of all nodes can be calculated as:
        ```
        αt = sigmoid(Us * tanh(Wh * HLt + Wq * qt + bs) + bu)
        ```
        where `αt = (αt1, αt2, ..., αtN )`, and `Us`, `Wh`, `Wq` are the
        learnable parameter matrixes. `bs`, `bu` are the bias. We use the
        non-linear function of `sigmoid` due to the possibility of existing
        multiple key joints.

        The hidden state `Hti` of node `vti` can also be represented as
        ```
        Hti = (1 + αti) · HLti
        ```
        The attention enhanced hidden state `Ht` will be fed into the next
        AGC-LSTM layer.

        Note that, at the last AGC-LSTM layer, the aggregation of all node
        features will serve as a global feature `Fgt` , and the weighted
        sum of focused nodes will serve as a local feature `Flt`:
        ```
        Fgt = sum(Hti)
        ```
        ```
        ...
        The global feature `Fgt` and local feature `Flt` are used to predict
        the class of human action.
        """
        # TODO: check if it is better to use activation
        w_h_q = self.attention_kernel_w * HLt
        qt = K.relu(K.sum(w_h_q, axis=1))

        # TODO: check if it is better to use activation
        w_h = self.attention_kernel_wh * HLt
        w_q = self.attention_kernel_wq * qt
        w_hq = K.tanh(w_h + w_q + self.attention_bias_bs)
        at = K.sigmoid(self.attention_kernel_us * w_hq + self.attention_bias_bu)

        # Global and local features:
        Ht = (1 + at) * HLt
        Fgt = K.sum(Ht, axis=1)
        Flt = K.sum(at * HLt, axis=1)
